...
import com.psr.seatservice.dto.program.response.BizProgramScheduleResponse;
...
public interface ProgramViewingRepository extends JpaRepository<ProgramViewing, ProgramViewingPK> {
    ...
    @Query(value = "select new com.psr.seatservice.dto.program.response.BizProgramScheduleResponse(" +
            "v.programNo, v.viewingDate, v.viewingTime, sum(case when b.status != '취소' then 1 else 0 end)) from ProgramViewing v " +
    ...
    List<BizProgramScheduleResponse> findViewingDateAndTimeAndCurrentParticipantsByProgramNum(Long programNo);
